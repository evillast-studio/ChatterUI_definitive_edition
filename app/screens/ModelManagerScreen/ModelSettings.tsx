...
const ModelSettings: React.FC<ModelSettingsProp> = ({ modelImporting, modelLoading, exit }) => {
    ...
    const [noStreamingMode, setNoStreamingMode] = useMMKVBoolean(AppSettings.NoStreamingMode)
        ...
        <Animated.ScrollView
            ...
            {config && (
                <>
                    ...
                    {/* GPU layers: quitado del UI. Confirmado en este dispositivo
                        (Kirin 710 / Mali-G51) que no aporta nada - el backend
                        OpenCL de llama.rn está tuneado para Adreno, no Mali, y
                        HiSilicon no expone un driver OpenCL de cómputo confiable
                        a apps de terceros. gpu_layers queda fijo en 0. */}

                    <ThemedTextInput
                        label="CPU Mask (cpu_mask)"
                        description="Fijar cores específicos para inferencia. Confirmado en este teléfono: 4-7 = A73 (2.2GHz, rápidos), 0-3 = A53 (1.7GHz, lentos)."
                    ...
                    />
                    <ThemedSwitch
                        label="Forzar CPU Mask (cpu_strict)"
                        description="Sin esto, cpu_mask es solo una sugerencia y Android puede migrar los hilos a los A53 igual (por térmica o ahorro de batería). Con esto activo, se fuerza a quedarse en los cores indicados."
                        value={config.cpu_strict}
                        onChangeValue={(value) => setConfig({ ...config, cpu_strict: value })}
                        disabled={modelImporting || modelLoading}
                    />
                    <ThemedSlider
            ...
            />
            <ThemedSwitch
                label="Máximo rendimiento (sin streaming en vivo)"
                description="No actualiza la pantalla token por token: acumula toda la respuesta y la muestra recién cuando termina de generar. Cero renders de React/Zustand compitiendo con la inferencia mientras corre. Vas a ver el indicador de 'escribiendo' fijo hasta que aparezca la respuesta completa de una sola vez."
                value={noStreamingMode}
                onChangeValue={setNoStreamingMode}
            />
