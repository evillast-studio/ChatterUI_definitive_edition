...
const runLocalCompletion = async (
    ...
    // Máximo rendimiento: cero actualizaciones de UI durante la generación entera.
    // Todo se acumula en memoria y se vuelca una sola vez al terminar - nada
    // compite por CPU/JS thread con la inferencia mientras corre.
    const noStreamingMode = mmkv.getBoolean(AppSettings.NoStreamingMode)
    let tokenBatch = ''
    ...
    const outputStream = (text: string) => {
        ...
        if (noStreamingMode) {
            // Nada de setBuffer/insertToBuffer hasta que termine: se acumula
            // solo en la variable local, sin tocar Zustand ni React para nada.
